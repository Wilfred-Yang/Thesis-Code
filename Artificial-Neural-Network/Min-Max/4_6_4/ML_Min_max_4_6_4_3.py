# -*- coding: utf-8 -*-
"""
Created on Sat Jan  5 21:02:28 2019
...
with open('input_layer_4_6_4_3.pickle', 'rb') as file:
    X = pickle.load(file)
    #print(X)
    X = np.transpose(X)
    ...
    X = X[:, 0:216]
    print(X.shape)
with open('output_layer_4_6_4_3.pickle', 'rb') as file:
    Y = pickle.load(file)
...
def Back_propogation(Parameters, caches, X, Y, A, T_Layer, lambd = 0.7):
    ...
    dA = -Y/A + (1-Y)/(1-A + 1e-8)  #logistic regression for only one neuron
    #dA = (A - Y) / (A * (1-A))
...
with open('weight_Min_max_4_6_4_3.pickle','wb') as file:    
