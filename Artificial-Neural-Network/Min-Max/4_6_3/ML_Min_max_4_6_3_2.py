# -*- coding: utf-8 -*-
"""
Created on Thu Dec  6 13:46:40 2018
...
with open('input_layer_4_6_3_2.pickle', 'rb') as file:
    X = pickle.load(file)
    #print(X)
    X = np.transpose(X)
    ...
    X = X[:, 0:72]
    print(X.shape)
with open('output_layer_4_6_3_2.pickle', 'rb') as file:
    Y = pickle.load(file)
...
def cost_function(A,Y, lambd = 0.7):
    ...
    Logi_reg = np.multiply(Y, np.log(A)) + np.multiply((1-Y),np.log(1-A))  #we can directly multiply it 
...
def Back_propogation(Parameters, caches, X, Y, A, T_Layer, lambd = 0.7):
    ...
    dA = -Y/A + (1-Y)/(1-A)  #logistic regression for only one neuron
    #dA = (A - Y) / (A * (1-A))
...
def nn_model(X,Y,num_epochs = 500000, optim_algorithm = "Adam", beta_1 = 0.9, beta_2 = 0.999, epsilson = 1e-8, learning_rate = 0.0035, print_cost = False):
    ...
    for i in range(0, num_epochs + 1):
        ...
        if print_cost and i % 10 == 0:
            print("Cost after epoch %i: %f" %(i, cost))        #%i, %f
        if print_cost and i % 10 == 0:
            costs.append(cost)
    '''
        if cost < low_cost:
...
Parameters = nn_model(X,Y,num_epochs = 2000,print_cost = True)
...
#with open('weight_Min_max_4_6_3_2.pickle','wb') as file:    
