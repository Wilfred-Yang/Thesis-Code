...
output_layer_b = np.zeros((len(output_layer), NCol ** 2), dtype = int)
...
for i in range(0, len(output_layer)):
    for j in range(0, NCol):
        ...
        if output_layer[i][j] == -1:
            ...
            y = x
    label = label_num[x][y]
    output_layer_b[i][label] = 1  

print(output_layer_b)     
output_layer_bi = np.transpose(output_layer_b)
'''
...
with open('output_layer_4_6_3_2.pickle' ,'wb') as file:
